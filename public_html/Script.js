...
document.getElementById('locationlist').style.display="block";
      ...
      document.getElementById('locsearch').style.opacity="0";
         ...
         var q = new Y.YQLRequest('select source,photo_id from flickr.photos.sizes where photo_id in (select id from flickr.photos.search where has_geo="true" and tags="'+textcon+'" and woe_id='+woeid+' and api_key="8a976c10ca749ae611e2a3ca9f0e7c9c" and group_id ="1463451@N25") and api_key="8a976c10ca749ae611e2a3ca9f0e7c9c"', function(r) {    
               if(r.query.results!=null){       
             
             
             var img=r.query.results.size[08].source;
            var imgp=r.query.results.size[08].source;
             Y.one('.mainbody').setStyle('background-image','url('+img+')');
             Y.one('#wallpaper').setStyle('background-image','url('+imgp+')');
           }else
          {
             var z =new Y.YQLRequest('select source,photo_id from flickr.photos.sizes where photo_id in (select id from flickr.photos.search where has_geo="true" and tags="'+textcon+'" and text="'+content+'" and api_key="8a976c10ca749ae611e2a3ca9f0e7c9c" and group_id ="1463451@N25") and api_key="8a976c10ca749ae611e2a3ca9f0e7c9c"', function(r) {    
                 var img=r.query.results.size[08].source;
                 var imgp=r.query.results.size[08].source;
             Y.one('.mainbody').setStyle('background-image','url('+img+')');
             Y.one('#wallpaper').setStyle('background-image','url('+imgp+')');
